...
export function WalletInfo() {
	...
	useEffect(() => {
		...
		log(
			'success',
			`Connected via @1sat/connect. Identity: ${identityKey?.slice(0, 16)}...`,
		)
		...
		ctx.wallet
			...
			.then((result) => {
				...
				log(
					'info',
					`Balance: ${total.toLocaleString()} sat (${result.outputs.length} outputs)`,
				)
			})
		...
		deriveDepositAddresses
			.execute(ctx, { startIndex: 0, count: 2 })
			.then((res) => {
			...
			.catch((err) =>
				log('error', `deriveDepositAddresses failed: ${err.message}`),
			)
	}, [ctx, identityKey, log])
	...
	return (
		<div style={card}>
			<div style={heading}>
				Wallet Details (@1sat/connect + @1sat/actions v0.0.69)
			</div>
			<div
				style={{
					display: 'grid',
			...
			>
				...
				<div>
					...
					<div style={mono}>
						{balance != null
							? `${balance.toLocaleString()} sat (${outputCount} outputs)`
							: 'Loading...'}
					</div>
				</div>
				<div>
