...
export function SignMessage() {
	...
	async function handleSign() {
		...
		log(
			'info',
			`signBsm: "${message.slice(0, 40)}${message.length > 40 ? '...' : ''}"`,
		)
		...
		<div style={card}>
			...
			{result && (
				<div style={successText}>
					<div>
						Address: <span style={mono}>{result.address}</span>
					</div>
					<div>
						PubKey: <span style={mono}>{result.pubKey}</span>
					</div>
					<div>
						Sig: <span style={mono}>{result.sig}</span>
					</div>
				</div>
			)}
