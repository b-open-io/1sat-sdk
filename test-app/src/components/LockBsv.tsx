import { getLockData, type LockData, lockBsv, unlockBsv } from '@1sat/actions'
...
export function LockBsv() {
	...
	async function handleGetLockData() {
		...
		try {
			...
			log(
				'success',
				`getLockData: ${res.totalLocked} locked, ${res.unlockable} unlockable, next unlock: ${res.nextUnlock}`,
			)
		} catch (err: unknown) {
	...
	return (
		<div style={card}>
			...
			{lockData && (
				...
				>
					Locked: {lockData.totalLocked} sat | Unlockable: {lockData.unlockable}{' '}
					sat | Next: block {lockData.nextUnlock}
				</div>
			)}
			...
			<div style={{ display: 'flex', gap: '0.5rem' }}>
				<button
					style={
						!ctx || loading || !amount || !blockHeight ? buttonDisabled : button
					}
