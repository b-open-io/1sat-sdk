	...
	async function handleInscribe() {
		...
		try {
			...
			if (file) {
				...
				log(
					'info',
					`inscribe: file ${file.name} (${mime}, ${buffer.byteLength} bytes)`,
				)
			} else {
	...
	return (
		<div style={card}>
			...
			>
				...
				/>
				Sign with BAP (Sigma) — intent{' '}
				{signWithBAP ? 'ordinal.inscribe-sigma' : 'ordinal.inscribe'}
			</label>
