...
export function OrdinalsList() {
	...
	const { log } = useLog()
	const [result, setResult] = useState<ListOrdinalsResult | null>(null)
	...
	async function handleFetch() {
		...
		setLoading(true)
		setError(null)
		log('info', 'listOrdinals...')

		try {
			const res = await listOrdinals.execute(ctx, { limit: 50 })
			setResult(res)
			log(
				'success',
				`listOrdinals: ${res.outputs.length} ordinal(s)${res.BEEF ? `, BEEF: ${res.BEEF.length} bytes` : ''}`,
			)
		} catch (err: unknown) {
			const msg = err instanceof Error ? err.message : String(err)
			setError(msg)
			log('error', `listOrdinals failed: ${msg}`)
		} finally {
			setLoading(false)
		}
	}
		...
		<div style={card}>
			...
			<div style={{ maxHeight: '400px', overflow: 'auto' }}>
				{result?.outputs.map((ord) => {
					...
					const assetId = readAssetIdTag(ord.tags)
...
function TextPreview({ url }: { url: string }) {
	...
	if (text === null)
		return <div style={{ color: '#555', fontSize: '0.7rem' }}>Loading...</div>
	return (
