  ...
  return (
    <div style={card}>
      ...
      <div style={{ maxHeight: '250px', overflow: 'auto' }}>
        {entries.map(tx => (
          <div key={`${tx.txid}-${tx.score}`} style={{ padding: '0.4rem 0', borderBottom: '1px solid #1a1a1a', fontSize: '0.75rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={mono}>{tx.txid.slice(0, 12)}...</span>
              <span style={{ color: tx.type === 'receive' ? '#22c55e' : '#ef4444', fontWeight: 600 }}>
                {tx.type === 'receive' ? '+' : '-'}${MneeClient.fromAtomicAmount(tx.amount).toFixed(2)}
              </span>
            </div>
            <div style={{ color: '#666', fontSize: '0.65rem' }}>
              {tx.status} | block {tx.height} | {tx.counterparties.map(c => `${c.address.slice(0, 8)}...`).join(', ')}
            </div>
          </div>
        ))}
      </div>
