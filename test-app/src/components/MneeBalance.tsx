import { type GetMneeBalanceResult, getMneeBalance } from '@1sat/actions'
...
export function MneeBalance() {
	...
	async function handleFetch() {
		...
		try {
			...
			log(
				'success',
				`MNEE balance: $${res.totalDecimal.toFixed(2)} (${res.balances.length} addresses)`,
			)
		} catch (err: unknown) {
		...
		<div style={card}>
			...
			{result && (
				<div style={{ marginBottom: '0.5rem' }}>
					<div
						style={{ fontSize: '1.2rem', fontWeight: 700, color: '#22c55e' }}
					>
					...
					{result.balances.map((b) => (
						...
						>
							<div style={{ display: 'flex', justifyContent: 'space-between' }}>
								...
								<span
									style={{ color: b.decimalAmount > 0 ? '#22c55e' : '#888' }}
								>
