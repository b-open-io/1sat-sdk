...
export interface IndexedOutput {
  ...
  events?: string[];
  owners?: Uint8Array[];
  spend_txid?: Uint8Array;
  data?: { [key: string]: Uint8Array };
}

export function encodeIndexedOutput(message: IndexedOutput): Uint8Array {
  let bb = popByteBuffer();
  _encodeIndexedOutput(message, bb);
  return toUint8Array(bb);
}

function _encodeIndexedOutput(message: IndexedOutput, bb: ByteBuffer): void {
  ...
  let $block_idx = message.block_idx;
  ...
  let array$owners = message.owners;
  if (array$owners !== undefined) {
    for (let value of array$owners) {
      writeVarint32(bb, 50);
  ...
  let $spend_txid = message.spend_txid;
...
export function decodeIndexedOutput(binary: Uint8Array): IndexedOutput {
  return _decodeIndexedOutput(wrapByteBuffer(binary));
}

function _decodeIndexedOutput(bb: ByteBuffer): IndexedOutput {
  let message: IndexedOutput = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      // repeated bytes owners = 6;
      case 6: {
        let values = message.owners || (message.owners = []);
...
export interface BeefParseResult {
  outputs?: IndexedOutput[];
  spends?: IndexedOutput[];
  txid?: Uint8Array;
  block_height?: number;
  block_idx?: Long;
}

export function encodeBeefParseResult(message: BeefParseResult): Uint8Array {
  let bb = popByteBuffer();
  _encodeBeefParseResult(message, bb);
  return toUint8Array(bb);
}

function _encodeBeefParseResult(message: BeefParseResult, bb: ByteBuffer): void {
  ...
  let array$spends = message.spends;
  if (array$spends !== undefined) {
    for (let value of array$spends) {
      writeVarint32(bb, 18);
      let nested = popByteBuffer();
      _encodeIndexedOutput(value, nested);
      writeVarint32(bb, nested.limit);
...
export function decodeBeefParseResult(binary: Uint8Array): BeefParseResult {
  return _decodeBeefParseResult(wrapByteBuffer(binary));
}

function _decodeBeefParseResult(bb: ByteBuffer): BeefParseResult {
  let message: BeefParseResult = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 2: {
        let limit = pushTemporaryLength(bb);
        let values = message.spends || (message.spends = []);
        values.push(_decodeIndexedOutput(bb));
        bb.limit = limit;
        break;
      }
