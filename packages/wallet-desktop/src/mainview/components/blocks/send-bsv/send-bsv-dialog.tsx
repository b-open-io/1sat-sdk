...
export function SendBsvDialog({
	...
	return (
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent
				className={cn(isCompact ? 'sm:max-w-sm' : 'sm:max-w-md', className)}
			>
				<DialogHeader className={cn(isCompact && 'pb-0')}>
					...
					{!isCompact && (
						<DialogDescription>
							Send Bitcoin SV to any address. Double-check the address and
							amount before confirming.
						</DialogDescription>
					)}
				</DialogHeader>

				<div className={cn('flex flex-col gap-4', isCompact ? 'py-2' : 'py-4')}>
					{/* Address input */}
					<div className={cn('flex flex-col gap-2', isCompact && 'gap-1.5')}>
						...
						<Input
							...
							className={cn('font-mono text-sm', isCompact && 'h-8 text-xs')}
							autoComplete="off"
							spellCheck={false}
						/>
					</div>
						...
						<div className="flex items-center gap-2">
							<div className="relative flex-1">
								<Input
									...
									disabled={isSending || !!result?.txid}
									className={cn('pr-14', isCompact && 'h-8 text-xs')}
					...
					{validationError && (
						<p
							className={cn('text-sm text-destructive', isCompact && 'text-xs')}
							role="alert"
						>
							{validationError}
						</p>
					...
					{result?.txid && (
						...
						>
							...
							<div className="min-w-0 flex flex-col gap-1">
								<p
									className={cn('text-sm font-medium', isCompact && 'text-xs')}
								>
									Sent successfully
								</p>
								<Badge
