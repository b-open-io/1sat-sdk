 ...
 */
function pickChallengePositions(wordCount: number): [number, number] {
...
export function useMnemonicFlow({
	...
	const isValid = useMemo(() => {
		switch (mode) {
			...
			case 'create':
				return confirmed && words.length > 0 && words.every((w) => w.length > 0)

			case 'import':
				return words.length === wordCount && words.every((w) => w.length > 0)
