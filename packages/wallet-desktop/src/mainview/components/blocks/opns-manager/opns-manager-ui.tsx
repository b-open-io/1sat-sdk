	...
	return (
		<>
			<div className="flex items-center gap-4 px-4 py-3">
				...
				>
					{isOperating ? <Loader2 data-icon className="animate-spin" /> : null}
...
export function OpnsManagerUI({
	...
	const handleConfirm = useCallback(async () => {
		...
		const handler = confirm.action === 'register' ? onRegister : onDeregister
		...
		return (
			<Card className={cn('w-full', className)}>
				...
				<CardContent className="flex flex-col items-center gap-2 py-10">
					<Globe className="size-10 text-destructive/60" aria-hidden="true" />
			...
			<Card className={cn('w-full', className)}>
				...
				<CardContent className="flex flex-col items-center gap-2 py-10">
					<Globe
						...
						aria-hidden="true"
					/>
					<p className="text-sm text-muted-foreground">No OpNS names found</p>
				</CardContent>
			</Card>
		)
	}
